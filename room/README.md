...
`room` connects Store6 to an existing Room database. Your DAO remains the
...
Use the same Store6 version for `core`, `room`, and
`testing`. Replace `<version>` with the release you consume:
...
Add `org.mobilenativefoundation.store:testing:<version>` to the test
...
room targets **androidx.room3 3.0.0** (Room's KMP-first line),
...
**Gaps vs `core`'s 12 targets: js, wasmJs, mingwX64, and iosX64.**
...
Use `sqldelight` or a custom source of truth on those targets meanwhile.
...
**Room 2.x is not supported by this artifact.** room launched on the `androidx.room3` line;
