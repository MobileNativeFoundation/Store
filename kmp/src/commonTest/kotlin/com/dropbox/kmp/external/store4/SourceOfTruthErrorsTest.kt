...
import com.dropbox.kmp.external.store4.SourceOfTruth.ReadException
import com.dropbox.kmp.external.store4.SourceOfTruth.WriteException
...
class SourceOfTruthErrorsTest {
    ...
    fun `GIVEN Source of Truth WHEN first write fails THEN it should keep reading from Fetcher`() =
        testScope.runBlockingTest {
            ...
            pipeline.stream(StoreRequest.cached(3, refresh = true)).emitsExactly(
                    StoreResponse.Loading(
                            origin = ResponseOrigin.Fetcher
                    ),
                    StoreResponse.Error.Exception(
    ...
    fun `Given Source of Truth with failing write WHEN a passive reader arrives THEN it should not get errors happened before`() =
        ...
        pipeline.stream(StoreRequest.skipMemory(3, refresh = true)).emitsExactly(
                StoreResponse.Data(
                        value = "b",
                        origin = ResponseOrigin.SourceOfTruth
                ),
