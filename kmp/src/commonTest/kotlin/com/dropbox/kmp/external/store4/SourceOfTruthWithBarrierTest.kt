...
import com.dropbox.kmp.external.store4.impl.PersistentSourceOfTruth
import com.dropbox.kmp.external.store4.impl.SourceOfTruthWithBarrier
...
class SourceOfTruthWithBarrierTest {
    ...
    fun simple() = testScope.runBlockingTest {
        ...
        assertEquals(expectedStoreData, collector.await())
    ...
    @Test
    fun `Given Source Of Truth WHEN read fails THEN error should propagate`() =
            testScope.runBlockingTest {
                val exception = RuntimeException("read fails")
                persister.postReadCallback = { key, value ->
                    throw exception
                }
                source.reader(1, CompletableDeferred(Unit)).emitsExactly(
    ...
    @Test
    fun `Given Source Of Truth WHEN read fails but then succeeds THEN error should propagate but also the value`() =
            testScope.runBlockingTest {
                var hasThrown = false
                val exception = RuntimeException("read fails")
    ...
    fun `Given Source Of Truth WHEN write fails THEN error should propogate`() {
        ...
        testScope.runBlockingTest {
            ...
            val eventsUntilFailure = listOf(
                    ...
                    StoreResponse.Error.Exception(
                            origin = ResponseOrigin.SourceOfTruth,
                            error = SourceOfTruth.WriteException(
                                    key = 1,
                                    value = failValue,
                                    cause = exception
                            )
            ...
            assertTrue { collected.containsAll(eventsUntilFailure) }
            advanceUntilIdle()
            assertTrue { collection.isActive }
            ...
            advanceUntilIdle()
            assertTrue {
                    collected.containsAll(eventsUntilFailure + StoreResponse.Data<String?>(
                            origin = ResponseOrigin.Fetcher,
                            value = "succeed"
                    )
                )
