...
class CacheTests {
  ...
  fun expireAfterAccess() = runTest {
    var timeNs = 0L
    val cache =
      CacheBuilder<String, String>().expireAfterAccess(100.milliseconds).ticker { timeNs }.build()
    cache.put("key", "value")
  ...
  fun expireAfterWrite() = runTest {
    var timeNs = 0L
    val cache =
      CacheBuilder<String, String>().expireAfterWrite(100.milliseconds).ticker { timeNs }.build()
    cache.put("key", "value")
