...
/**
 * A class that represents a caching system with collection decomposition. Manages data with utility
 * functions to get, invalidate, and add items to the cache. Depends on [StoreMultiCacheAccessor]
...
>(
  ...
  singlesCache: Cache<StoreKey.Single<Id>, Single> =
    CacheBuilder<StoreKey.Single<Id>, Single>().build(),
  collectionsCache: Cache<StoreKey.Collection<Id>, Collection> =
    CacheBuilder<StoreKey.Collection<Id>, Collection>().build(),
) : Cache<Key, Output> {
  private val accessor =
    StoreMultiCacheAccessor(singlesCache = singlesCache, collectionsCache = collectionsCache)
