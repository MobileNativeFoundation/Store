...
/**
 * Responsible for managing and accessing cached data. Provides functionality to retrieve, store,
 * and invalidate single items and collections of items. All operations are thread-safe, ensuring
 * safe usage across multiple threads.
 *
 * The thread safety of this class is ensured through the use of synchronized blocks. Synchronized
 * blocks guarantee only one thread can execute any of the methods at a time. This prevents
 * concurrent modifications and ensures consistency of the data.
 *
...
>(
  ...
  fun getCollection(key: StoreKey.Collection<Id>): Collection? =
    synchronized(this) { collectionsCache.getIfPresent(key) }
  ...
  fun getSingle(key: StoreKey.Single<Id>): Single? =
    synchronized(this) { singlesCache.getIfPresent(key) }
  ...
  /**
   * Calculates the total count of items in the cache, including both single items and items in
   * collections.
   *
   * This operation is thread-safe.
   *
