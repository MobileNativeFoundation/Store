...
internal class LocalCache<K : Any, V : Any>(builder: CacheBuilder<K, V>) {
  ...
  internal class LocalManualCache<K : Any, V : Any>
  private constructor(private val localCache: LocalCache<K, V>) : Cache<K, V> {
  ...
  companion object {
    ...
    private val UNSET: ValueReference<Any, Any> =
      object : ValueReference<Any, Any> {
    ...
    private val DISCARDING_QUEUE: MutableQueue<Any> =
      object : MutableQueue<Any> {
  ...
  init {
    this.maxWeight =
      when {
        builder.expireAfterAccess == Duration.ZERO || builder.expireAfterWrite == Duration.ZERO ->
          0L
    ...
    this.expireAfterAccessNanos =
      (if (builder.expireAfterAccess == Duration.INFINITE) Duration.ZERO
        else builder.expireAfterAccess)
        .inWholeNanoseconds

    this.expireAfterWriteNanos =
      (if (builder.expireAfterWrite == Duration.INFINITE) Duration.ZERO
        else builder.expireAfterWrite)
        .inWholeNanoseconds
