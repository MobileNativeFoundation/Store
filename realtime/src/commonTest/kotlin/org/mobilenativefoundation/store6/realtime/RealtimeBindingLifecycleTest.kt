...
class RealtimeBindingLifecycleTest {
    ...
    fun consume_appliesMessagesInOrder_thenCompletes() = runTest {
        ...
        try {
            ...
            assertEquals(listOf("captureFreshness", "applyAcknowledgement", "markStale", "confirmFresh"), handle.events)
        } finally {
            store.close()
        }
    }
    ...
    fun consume_propagatesError_andStops() = runTest {
        ...
        val handle =
            object : StoreWriteHandle<RealtimeTestKey, String> {
                ...
                override suspend fun applyAcknowledgement(
                    ...
                    events += "applyAcknowledgement:$value"
        ...
        try {
            ...
            assertEquals(listOf("applyAcknowledgement:ok", "applyAcknowledgement:boom"), handle.events)
        } finally {
            store.close()
        }
    }
    ...
    @Test
    fun concurrentApply_serializesAcknowledgements() = runTest {
        val store = store<RealtimeTestKey, String> { fetcher { "fetched" } }
        ...
        val handle =
            object : StoreWriteHandle<RealtimeTestKey, String> {
                ...
                override suspend fun applyAcknowledgement(
                    ...
                    events += "acknowledgement-start:$value"
                    applies += 1
                    if (applies == 1) {
                        applyStarted.complete(Unit)
                        releaseApply.await()
                    }
                    events += "acknowledgement-end:$value"
                }
        ...
        try {
            ...
            first.await()
            second.await()
            assertEquals(listOf("acknowledgement-start:a", "acknowledgement-end:a", "acknowledgement-start:b", "acknowledgement-end:b"), handle.events)
        } finally {
            store.close()
        }
    }
    ...
    @Test
    fun defaultAcknowledgementHandle_marksStaleBeforeUpsertWithoutConfirmation() = runTest {
        val store = store<RealtimeTestKey, String> { fetcher { "fetched" } }
        val events = mutableListOf<String>()
        val handle = object : StoreWriteHandle<RealtimeTestKey, String> {
            ...
            override suspend fun confirmFresh(key: RealtimeTestKey, etag: String?) {
                error("A default acknowledgement handle has no freshness authority")
            }
        }
        try {
            RealtimeBinding(store, handle).apply(RealtimeMessage.Upsert(RealtimeTestKey("default"), "pushed", "etag"))
            assertEquals(listOf("markStale", "apply:pushed"), events)
        } finally {
            store.close()
        }
    }
