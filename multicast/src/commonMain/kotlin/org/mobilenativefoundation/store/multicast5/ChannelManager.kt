 ...
 *
 * The [ChannelManager] will start the upstream from the given [upstream] [Flow] if there is no
 * active upstream and there's at least one downstream that has not received a value.
 */
internal class StoreChannelManager<T>(
  ...
  private inner class Actor : StoreRealActor<ChannelManager.Message<T>>(scope) {
    ...
    /**
     * Tracks whether we've ever dispatched value or error from the current producer. Reset when
     * producer finishes.
     */
    private var dispatchedValue: Boolean = false

    /**
     * The ack for the very last message we've delivered. When a new downstream comes and buffer is
     * 0, we ack this message so that new downstream can immediately start receiving values instead
     * of waiting for values that it'll never receive.
     */
    ...
    /**
     * We are closing. Do a cleanup on existing channels where we'll close them and also decide on
     * the list of leftovers.
     */
    ...
    private suspend fun doDispatchValue(msg: Message.Dispatch.Value<T>) {
      ...
      channels.forEach { it.dispatchValue(msg) }
    }
    ...
    private suspend fun doAdd(msg: Message.AddChannel<T>) {
      ...
      addEntry(
        entry =
          ChannelManager.ChannelEntry(channel = msg.channel, piggybackOnly = msg.piggybackOnly)
      )
    ...
    private suspend fun addEntry(entry: ChannelManager.ChannelEntry<T>) {
      val new = channels.none { it.hasChannel(entry) }
      check(new) { "$entry is already in the list." }
      channels.add(entry)
      if (buffer.items.isNotEmpty()) {
        // if there is anything in the buffer, send it
        buffer.items.forEach { entry.dispatchValue(it) }
      } else {
