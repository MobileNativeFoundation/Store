...
class RxFlowableStoreTest {
  ...
  @Test
  fun simpleTest() {
    val testSubscriber1 = store.observe(StoreReadRequest.fresh(3)).subscribeOn(testScheduler).test()
    ...
    val testSubscriber2 =
      store.observe(StoreReadRequest.cached(3, false)).subscribeOn(testScheduler).test()
    testScheduler.triggerActions()
    testSubscriber2
      .awaitCount(2)
      .assertValues(
    ...
    val testSubscriber3 = store.observe(StoreReadRequest.fresh(3)).subscribeOn(testScheduler).test()
    testScheduler.triggerActions()
    testSubscriber3
      .awaitCount(3)
      .assertValues(
