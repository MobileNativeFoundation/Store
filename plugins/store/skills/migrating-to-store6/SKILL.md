...
- **Persistence:** the seam `SourceOfTruth<K, V>` is installed via `persistence(...)`. Both are `@ExperimentalStoreApi`, and implementing the interface additionally requires opt-in to `DelicateStoreApi`. Prefer the `room` or `sqldelight` adapters. Validate custom implementations with the `testing` contract kit.
