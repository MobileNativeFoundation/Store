...
internal class RealMutableStore<Key : Any, Network : Any, Output : Any, Local : Any>(
  ...
  override fun <Response : Any> stream(
  ...
  ): Flow<StoreReadResponse<Output>> = flow {
    ...
    when (val eagerConflictResolutionResult = tryEagerlyResolveConflicts<Response>(request.key)) {
      // TODO(#678): Many use cases will not want to pull immediately after failing to push local
      // changes.
      // We should enable configuration of conflict resolution strategies, such as logging,
      // retrying, canceling.
          ...
          try {
            ...
            when (updaterResult) {
              is UpdaterResult.Error.Exception ->
                StoreWriteResponse.Error.Exception(updaterResult.error)
              is UpdaterResult.Error.Message ->
                StoreWriteResponse.Error.Message(updaterResult.message)
              ...
              is UpdaterResult.Success.Untyped ->
                StoreWriteResponse.Success.Untyped(updaterResult.value)
            }
  ...
  ): StoreWriteResponse = stream(flowOf(request)).first()

  private suspend fun <Response : Any> tryUpdateServer(
    request: StoreWriteRequest<Key, Output, Response>
  ): UpdaterResult {
  ...
  private suspend fun <Response : Any> updateWriteRequestQueue(
    ...
    storeLock.withLock { keyToWriteRequestQueue[key] = nextWriteRequestQueue }
  }
  ...
  private suspend fun getLatestWriteRequest(key: Key): StoreWriteRequest<Key, Output, *> {
    ...
    return try {
      val queue = getQueue(key)
      require(queue.isNotEmpty()) { "No writes found for key=$key." }
      queue.last()
    } finally {
  ...
  private suspend fun <Response : Any> addWriteRequestToQueue(
    writeRequest: StoreWriteRequest<Key, Output, Response>
  ) = withWriteRequestQueueLock(writeRequest.key) { add(writeRequest) }
  ...
  /**
   * Ensures that [keyToThreadSafety] and [keyToWriteRequestQueue] have entries for [key]. We only
   * hold [storeLock] while touching these two maps, then release it immediately.
   */
  ...
  /**
   * Retrieves the [ThreadSafety] object for [key] without reinitializing it, since [safeInitStore]
   * handles creation. We do a quick [storeLock] read then release it without nesting per-key locks
   * inside [storeLock].
   */
  private suspend fun getThreadSafety(key: Key): ThreadSafety {
    return storeLock.withLock {
      requireNotNull(keyToThreadSafety[key]) { "ThreadSafety not initialized for key=$key." }
    }
  }
  ...
  private suspend fun getQueue(key: Key): WriteRequestQueue<Key, Output, *> {
    return storeLock.withLock {
      requireNotNull(keyToWriteRequestQueue[key]) { "No write request queue found for key=$key." }
    }
  }
}
