 ...
 *
 * See [ofResult] for easily translating from a regular `suspend` function. See [ofFlow], [of] for
 * easily translating to [FetcherResult] (and automatically transforming exceptions into
 * [FetcherResult.Error]).
 *
...
interface Fetcher<Key : Any, Network : Any> {
  ...
  companion object {
    ...
    /**
     * Creates a [Fetcher] with a [fallback] from a [flowFactory]. Use instead of [ofResultFlow] if
    ...
    fun <Key : Any, Network : Any> ofResultFlowWithFallback(
      ...
      FactoryFetcherWithFallback(name = name, factory = flowFactory, fallback = fallback)
    ...
    /**
     * Creates a [Fetcher] with a [fallback] from a non-Flow source. Use instead of [ofResult] if
     * implementing fallback mechanisms.
     *
     * @param name Unique name to enable differentiation of fetchers.
     */
     ...
     *
     * Emitted values will be wrapped in [FetcherResult.Data]. if an exception disrupts the flow
     * then it will be wrapped in [FetcherResult.Error]. Exceptions thrown in [flowFactory] itself
     * are not caught and will be returned to the caller.
     *
     * Use when creating a [Store] that fetches objects in a multiple responses per request network
     * protocol (e.g Web Sockets).
     *
    ...
    /**
     * Creates a [Fetcher] with a [fallback] from a [flowFactory]. Use instead of [ofFlow] if
     * implementing fallback mechanisms.
     *
     * @param name Unique name to enable differentiation of fetchers
     */
    ...
    /**
     * "Creates" a [Fetcher] from a non-[Flow] source and translate the results to a
     * [FetcherResult].
     *
     * Emitted values will be wrapped in [FetcherResult.Data]. if an exception disrupts the flow
     * then it will be wrapped in [FetcherResult.Error]
     *
     * Use when creating a [Store] that fetches objects in a single response per request network
     * protocol (e.g Http).
     *
    ...
    /**
     * Creates a [Fetcher] with a [fallback] from a non-Flow source. Use instead of [of] if
     * implementing fallback mechanisms.
     *
     * @param name Unique name to enable differentiation of fetchers
     */
    fun <Key : Any, Network : Any> withFallback(
      ...
      fallback: Fetcher<Key, Network>,
      fetch: suspend (key: Key) -> Network,
    ): Fetcher<Key, Network> = ofFlowWithFallback(name, fallback, fetch.asFlow())

    private fun <Key : Any, Network : Any> (suspend (key: Key) -> Network).asFlow() = { key: Key ->
      flow { emit(invoke(key)) }
    }
