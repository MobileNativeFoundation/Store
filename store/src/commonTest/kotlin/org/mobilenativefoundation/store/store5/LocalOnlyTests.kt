...
class LocalOnlyTests {
  ...
  fun givenEmptyMemoryCacheThenCacheOnlyRequestReturnsNoNewData() =
    testScope.runTest {
      val store =
        StoreBuilder.from(
            Fetcher.of { _: Int -> throw RuntimeException("Fetcher shouldn't be hit") }
          )
          .cachePolicy(MemoryPolicy.builder<Any, Any>().build())
          .build()
    ...
    testScope.runTest {
      ...
      val store =
        StoreBuilder.from(
          ...
          .cachePolicy(MemoryPolicy.builder<Any, Any>().setExpireAfterWrite(Duration.ZERO).build())
          .build()
      val a = store.get(0)
      assertEquals("result", a)
      assertEquals(1, fetcherHitCounter.value)
