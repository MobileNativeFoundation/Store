...
class SourceOfTruthWithBarrierTests {
  ...
  private val delegate: SourceOfTruth<Int, String, String> =
    PersistentSourceOfTruth(
      realReader = { key -> flow { emit(persister.read(key)) } },
      realWriter = persister::write,
      realDelete = persister::deleteByKey,
      realDeleteAll = persister::deleteAll,
    )
  private val source = SourceOfTruthWithBarrier<Int, String, String, String>(delegate = delegate)
  ...
  fun simple() =
    testScope.runTest {
      ...
      launch { source.reader(1, CompletableDeferred(Unit)).take(2).collect { collection.add(it) } }
      delay(500)
      source.write(1, "a")
      advanceUntilIdle()
      assertEquals(
        listOf<StoreReadResponse<String?>>(
          StoreReadResponse.Data(origin = StoreReadResponseOrigin.SourceOfTruth, value = null),
          StoreReadResponse.Data(origin = StoreReadResponseOrigin.Fetcher(), value = "a"),
        ),
        collection,
      )
  ...
  fun preAndPostWrites() =
    testScope.runTest {
      ...
      assertEquals(
        listOf<StoreReadResponse<String?>>(
          StoreReadResponse.Data(origin = StoreReadResponseOrigin.SourceOfTruth, value = "a"),
          StoreReadResponse.Data(origin = StoreReadResponseOrigin.Fetcher(), value = "b"),
        ),
        collection,
      )
  ...
  fun givenSourceOfTruthWhenReadFailsThenErrorShouldPropagate() =
    testScope.runTest {
      ...
      persister.postReadCallback = { key, value -> throw exception }
  ...
  fun givenSourceOfTruthWhenReadFailsButThenSucceedsThenErrorShouldPropagateButAlsoTheValue() =
    testScope.runTest {
      ...
      val collection = async { reader.collect { collected.add(it) } }
      advanceUntilIdle()
      assertEquals(
        StoreReadResponse.Error.Exception(
          origin = StoreReadResponseOrigin.SourceOfTruth,
    ...
    testScope.runTest {
      ...
      val eventsUntilFailure =
        listOf(
          ...
          StoreReadResponse.Error.Exception(
            origin = StoreReadResponseOrigin.SourceOfTruth,
            error = WriteException(key = 1, value = failValue, cause = exception),
          ),
          StoreReadResponse.Data<String?>(
            origin = StoreReadResponseOrigin.SourceOfTruth,
            value = null,
