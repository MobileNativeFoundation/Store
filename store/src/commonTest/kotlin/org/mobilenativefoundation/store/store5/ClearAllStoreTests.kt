...
class ClearAllStoreTests {
  ...
  @Test
  fun callingClearAllOnStoreWithPersisterAndNoInMemoryCacheDeletesAllEntriesFromThePersister() =
    testScope.runTest {
      val store =
      ...
      advanceUntilIdle()
      assertEquals(
        StoreReadResponse.Data(origin = StoreReadResponseOrigin.Fetcher(), value = value2),
        responseTwoA,
      )
      ...
      advanceUntilIdle()
      assertEquals(
        StoreReadResponse.Data(origin = StoreReadResponseOrigin.SourceOfTruth, value = value1),
        responseOneB,
      )
      val responseTwoB = store.getData(key2)
      advanceUntilIdle()
      assertEquals(
        StoreReadResponse.Data(origin = StoreReadResponseOrigin.SourceOfTruth, value = value2),
        responseTwoB,
      )
  ...
  fun callingClearAllOnStoreWithInMemoryCacheAndNoPersisterDeletesAllEntriesFromTheInMemoryCache() =
    testScope.runTest {
      val store = StoreBuilder.from(fetcher = fetcher).scope(testScope).build()
      ...
      assertEquals(
        StoreReadResponse.Data(origin = StoreReadResponseOrigin.Cache, value = value1),
        store.getData(key1),
      )
      assertEquals(
        StoreReadResponse.Data(origin = StoreReadResponseOrigin.Cache, value = value2),
        store.getData(key2),
      )
