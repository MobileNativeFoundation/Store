...
class ClearStoreByKeyTests {
  ...
  @Test
  fun callingClearWithKeyOnStoreWithPersisterWithNoInMemoryCacheDeletesTheEntryAssociatedWithTheKeyFromThePersister() =
    testScope.runTest {
      val key = "key"
      val value = 1
      val store =
      ...
      assertEquals(
        Data(origin = StoreReadResponseOrigin.Fetcher(), value = value),
        store.getData(key),
      )

      // should receive data from persister
      assertEquals(
        Data(origin = StoreReadResponseOrigin.SourceOfTruth, value = value),
        store.getData(key),
      )
  ...
  @Test
  fun callingClearWithKeyOStoreWithInMemoryCacheNoPersisterDeletesTheEntryAssociatedWithTheKeyFromTheInMemoryCache() =
    testScope.runTest {
      val key = "key"
      val value = 1
      val store = StoreBuilder.from(fetcher = Fetcher.of { value }).scope(testScope).build()
      ...
      assertEquals(Data(origin = StoreReadResponseOrigin.Cache, value = value), store.getData(key))
  ...
  fun callingClearWithKeyOnStoreHasNoEffectOnExistingEntriesAssociatedWithOtherKeysInTheInMemoryCacheOrPersister() =
    testScope.runTest {
      ...
      assertEquals(
        Data(origin = StoreReadResponseOrigin.Cache, value = value2),
        store.getData(key2),
      )
    }
}
