...
class SimplePersisterAsFlowable<Key : Any, Output : Any>(
  ...
  fun flowReader(key: Key): Flow<Output?> = flow {
    versionTracker.keyFlow(key).collect { emit(reader(key)) }
  }
...
internal class KeyTracker<Key> {
  ...
  suspend fun invalidate(key: Key) {
    lock.withLock { flows[key] }?.flow?.emit(Unit)
  }
  ...
  suspend fun keyFlow(key: Key): Flow<Unit> {
    ...
    // cleaned up
    return flow {
      val keyFlow = lock.withLock { flows.getOrPut(key) { KeyFlow() }.also { it.acquire() } }
      emit(Unit)
      try {
...
fun <Key : Any, Output : Any> InMemoryPersister<Key, Output>.asFlowable() =
  SimplePersisterAsFlowable(reader = this::read, writer = this::write)
