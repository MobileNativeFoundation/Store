...
/**
 * Helper factory that will return [StoreReadResponse.Data] for [key] if it is cached otherwise will
...
suspend fun <Key : Any, Output : Any> Store<Key, Output>.getData(key: Key) =
  stream(StoreReadRequest.cached(key, refresh = false))
    ...
    .mapIndexed { index, value -> value }
    .first()
    .let { StoreReadResponse.Data(it.requireData(), it.origin) }
