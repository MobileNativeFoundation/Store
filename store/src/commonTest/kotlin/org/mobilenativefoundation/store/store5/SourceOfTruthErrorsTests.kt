...
class SourceOfTruthErrorsTests {
  ...
  fun givenSourceOfTruthWhenWriteFailsThenExceptionShouldBeSendToTheCollector() =
    testScope.runTest {
      ...
      val fetcher = FakeFetcher(3 to "a", 3 to "b")
      val pipeline =
        StoreBuilder.from<Int, String, String>(
          ...
          .scope(testScope)
          .build()
      persister.preWriteCallback = { _, _ -> throw TestException("i fail") }

      pipeline.stream(StoreReadRequest.fresh(3)).test {
        assertEquals(StoreReadResponse.Loading(StoreReadResponseOrigin.Fetcher()), awaitItem())

        assertEquals(
          StoreReadResponse.Error.Exception(
            error = WriteException(key = 3, value = "a", cause = TestException("i fail")),
  ...
  fun givenSourceOfTruthWhenReadFailsThenExceptionShouldBeSendToTheCollector() =
    testScope.runTest {
      ...
      persister.postReadCallback = { _, value -> throw TestException(value ?: "null") }

      pipeline.stream(StoreReadRequest.cached(3, refresh = false)).test {
        assertEquals(
          StoreReadResponse.Error.Exception(
            error = ReadException(key = 3, cause = TestException("null")),
            origin = StoreReadResponseOrigin.SourceOfTruth,
          ),
          awaitItem(),
        )
        ...
        assertEquals(
          StoreReadResponse.Error.Exception(
            error = ReadException(key = 3, cause = TestException("a")),
            origin = StoreReadResponseOrigin.SourceOfTruth,
          ),
          awaitItem(),
        )
  ...
  fun givenSourceOfTruthWhenFirstWriteFailsThenItShouldKeepReadingFromFetcher() =
    testScope.runTest {
      val persister = InMemoryPersister<Int, String>()
      val fetcher = Fetcher.ofFlow { _: Int -> flowOf("a", "b", "c", "d") }
      val pipeline =
        StoreBuilder.from<Int, String, String>(
            fetcher = fetcher,
            sourceOfTruth = persister.asSourceOfTruth(),
      ...
      pipeline.stream(StoreReadRequest.cached(3, refresh = true)).test {
        ...
        assertEquals(
          StoreReadResponse.Error.Exception(
            error = WriteException(key = 3, value = "a", cause = TestException("a")),
            origin = StoreReadResponseOrigin.SourceOfTruth,
          ),
          awaitItem(),
        )
        ...
        assertEquals(
          StoreReadResponse.Data(value = "b", origin = StoreReadResponseOrigin.Fetcher()),
          awaitItem(),
        )

        assertEquals(
          StoreReadResponse.Error.Exception(
            error = WriteException(key = 3, value = "c", cause = TestException("c")),
            origin = StoreReadResponseOrigin.SourceOfTruth,
          ),
          awaitItem(),
        )
        ...
        assertEquals(
          StoreReadResponse.Data(value = "b", origin = StoreReadResponseOrigin.SourceOfTruth),
          awaitItem(),
        )

        assertEquals(
          StoreReadResponse.Data(value = "d", origin = StoreReadResponseOrigin.Fetcher()),
          awaitItem(),
        )
      }
    }
  ...
  //    @Test
  //    fun
  // givenSourceOfTruthWithFailingWriteWhenAPassiveReaderArrivesThenItShouldReceiveTheNewWriteError() = testScope.runTest {
  //        val persister = InMemoryPersister<Int, String>()
  ...
  fun givenSourceOfTruthWithFailingWriteWhenAPassiveReaderArrivesThenItShouldNotGetErrorsHappenedBefore() =
    testScope.runTest {
      ...
      val collector = launch {
        pipeline.stream(StoreReadRequest.cached(3, refresh = true)).toList() // keep collection hot
      }
  ...
  //    @Test
  //    fun
  // givenSourceOfTruthWithFailingWriteWhenAFreshValueReaderArrivesThenItShouldNotGetDiskErrorsFromAPendingWrite() = testScope.runTest {
  //        val persister = InMemoryPersister<Int, String>()
    ...
    testScope.runTest {
      ...
      pipeline.stream(StoreReadRequest.cached(3, refresh = true)).test {
        assertEquals(
          StoreReadResponse.Error.Exception(
            origin = StoreReadResponseOrigin.SourceOfTruth,
            error = ReadException(key = 3, cause = TestException("first read")),
          ),
          awaitItem(),
        )
        ...
        assertEquals(
          StoreReadResponse.Data(value = "a", origin = StoreReadResponseOrigin.Fetcher()),
          awaitItem(),
        )
      }
    }
