  ...
  fun givenEmptyStoreWhenSuccessFromPrimaryApiThenStoreReadResponseOfPrimaryApiResult() =
    testScope.runTest {
      ...
      val secondaryApiFetcher =
        Fetcher.withFallback(secondaryApi.name, hardcodedPagesFetcher) { key ->
          secondaryApi.get(key)
        }

      val store =
        StoreBuilder.from(
            fetcher =
              Fetcher.withFallback(api.name, secondaryApiFetcher) { key ->
                api.fetch(key, fail, ttl)
              },
            sourceOfTruth =
              SourceOfTruth.of(
