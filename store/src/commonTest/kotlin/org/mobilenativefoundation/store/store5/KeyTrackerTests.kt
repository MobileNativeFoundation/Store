...
class KeyTrackerTests {
  ...
  fun dontSkipInvalidations() =
    scope1.runTest {
      val collection = scope2.async { subject.keyFlow('b').take(2).toList() }
  ...
  fun multipleScopes() =
    scope1.runTest {
      val keys = 'a'..'z'
      val collections =
        keys.associate { key -> key to scope2.async { subject.keyFlow(key).take(2).toList() } }
      scope2.advanceUntilIdle()
      assertEquals(26, subject.activeKeyCount())

      scope2.advanceUntilIdle()
      keys.forEach { subject.invalidate(it) }
      scope2.advanceUntilIdle()

      collections.forEach { (_, deferred) -> assertEquals(true, deferred.isCompleted) }
      assertEquals(0, subject.activeKeyCount())
    }
  ...
  fun multipleObservers() =
    scope1.runTest {
      val collections = (0..4).map { scope2.async { subject.keyFlow('b').take(2).toList() } }
      scope2.advanceUntilIdle()
      assertEquals(1, subject.activeKeyCount())
      scope2.advanceUntilIdle()
      subject.invalidate('a')
      ...
      subject.invalidate('c')
      scope2.advanceUntilIdle()
      collections.forEach { collection -> assertEquals(true, collection.isCompleted) }
      assertEquals(0, subject.activeKeyCount())
    }
