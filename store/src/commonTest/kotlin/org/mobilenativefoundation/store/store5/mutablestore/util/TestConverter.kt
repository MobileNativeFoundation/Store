...
class TestConverter<Network : Any, Local : Any, Output : Any>(
  ...
  fun wheneverNetwork(network: Network, block: () -> Local) {
    networkToLocalMap[network] = block()
  }

  fun wheneverOutput(output: Output, block: () -> Local) {
    outputToLocalMap[output] = block()
  }

  override fun fromNetworkToLocal(network: Network): Local {
    return networkToLocalMap[network]
      ?: defaultNetworkToLocalConverter?.invoke(network)
      ?: network as Local
  }

  override fun fromOutputToLocal(output: Output): Local {
    return outputToLocalMap[output]
      ?: defaultOutputToLocalConverter?.invoke(output)
      ?: output as Local
  }
}
