...
class FlowStoreTests {
  ...
  fun getAndFresh() =
    testScope.runTest {
      ...
      assertEquals(
        ...
        listOf(
          Loading(origin = StoreReadResponseOrigin.Fetcher()),
          Data(value = "three-2", origin = StoreReadResponseOrigin.Fetcher()),
        ),
      )

      assertEquals(
        pipeline.stream(StoreReadRequest.cached(3, refresh = false)).take(1).toList(),
        listOf(Data(value = "three-2", origin = StoreReadResponseOrigin.Cache)),
      )
    }
  ...
  fun getAndFresh_withPersister() =
    testScope.runTest {
      ...
      val pipeline =
        StoreBuilder.from(fetcher = fetcher, sourceOfTruth = persister.asSourceOfTruth())
          .buildWithTestScope()

      pipeline.stream(StoreReadRequest.cached(3, refresh = false)).test {
        assertEquals(Loading(origin = StoreReadResponseOrigin.Fetcher()), awaitItem())
      ...
      pipeline.stream(StoreReadRequest.cached(3, refresh = false)).test {
        assertEquals(Data(value = "three-1", origin = StoreReadResponseOrigin.Cache), awaitItem())
        ...
        assertEquals(
          Data(value = "three-1", origin = StoreReadResponseOrigin.SourceOfTruth),
          awaitItem(),
        )
      }
      ...
      pipeline.stream(StoreReadRequest.cached(3, refresh = false)).test {
        assertEquals(Data(value = "three-2", origin = StoreReadResponseOrigin.Cache), awaitItem())

        assertEquals(
          Data(value = "three-2", origin = StoreReadResponseOrigin.SourceOfTruth),
          awaitItem(),
        )
      }
    }
  ...
  fun streamAndFresh() =
    testScope.runTest {
      val fetcher = FakeFetcher(3 to "three-1", 3 to "three-2")
      val pipeline = StoreBuilder.from(fetcher = fetcher).buildWithTestScope()
  ...
  fun flowingFetcher() =
    testScope.runTest {
      val fetcher = FakeFlowingFetcher(3 to "three-1", 3 to "three-2")
      val persister = InMemoryPersister<Int, String>()
  ...
  fun diskChangeWhileNetworkIsFlowing_simple() =
    testScope.runTest {
      ...
      pipeline.stream(StoreReadRequest.cached(3, refresh = true)).test {
        ...
        assertEquals(
          Data(value = "local-1", origin = StoreReadResponseOrigin.SourceOfTruth),
          awaitItem(),
        )
  ...
  fun diskChangeWhileNetworkIsFlowing_overwrite() =
    testScope.runTest {
      ...
      pipeline.stream(StoreReadRequest.cached(3, refresh = true)).test {
        ...
        assertEquals(
          Data(value = "local-2", origin = StoreReadResponseOrigin.SourceOfTruth),
          awaitItem(),
        )
  ...
  fun givenSourceOfTruthWhenStreamFreshDataReturnsNoDataFromFetcherThenFetchReturnsNoDataAndCachedValuesAreReceived() =
    testScope.runTest {
      ...
      pipeline.stream(StoreReadRequest.fresh(3)).test {
        ...
        assertEquals(Data(value = "local-1", origin = StoreReadResponseOrigin.Cache), awaitItem())
  ...
  fun givenNoSourceOfTruthWhenStreamFreshDataReturnsNoDataFromFetcherThenFetchReturnsNoDataAndCachedValuesAreReceived() =
    testScope.runTest {
      ...
      pipeline.stream(StoreReadRequest.fresh(3)).test {
        ...
        assertEquals(Data(value = "remote-1", origin = StoreReadResponseOrigin.Cache), awaitItem())
      }
    }
  ...
  fun givenNoSourceOfTruthAndCacheHitWhenStreamCachedDataWithoutRefreshThenNoFetchIsTriggeredAndReceivesFollowingNetworkUpdates() =
    testScope.runTest {
      ...
      val collection = launch {
        store.stream(StoreReadRequest.cached(3, refresh = false)).collect { secondCollect.add(it) }
      }
      testScope.runCurrent()
      assertEquals(1, secondCollect.size)
      assertContains(secondCollect, Data(value = "three-1", origin = StoreReadResponseOrigin.Cache))
  ...
  fun testSlowFirstCollectorGetsAllFetchUpdatesOthersGetCacheAndLatestFetchResult() =
    testScope.runTest {
      val fetcher = FakeFetcher(3 to "three-1", 3 to "three-2", 3 to "three-3")
      val pipeline = StoreBuilder.from(fetcher = fetcher).buildWithTestScope()
      ...
      pipeline.stream(StoreReadRequest.cached(3, refresh = true)).test {
        assertEquals(Data(origin = StoreReadResponseOrigin.Cache, value = "three-1"), awaitItem())
        assertEquals(Loading(origin = StoreReadResponseOrigin.Fetcher()), awaitItem())
      ...
      pipeline.stream(StoreReadRequest.cached(3, refresh = true)).test {
        assertEquals(Data(origin = StoreReadResponseOrigin.Cache, value = "three-2"), awaitItem())
  ...
  suspend fun Store<Int, Int>.get(key: Int) =
    get(StoreReadRequest.cached(key = key, refresh = false))
