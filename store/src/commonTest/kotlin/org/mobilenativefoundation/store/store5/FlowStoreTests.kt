...
class FlowStoreTests {
        ...
        testScope.runTest {
            ...
            val pipeline =
                StoreBuilder.from(
                    fetcher = fetcher,
                    sourceOfTruth = persister.asSourceOfTruth(),
                ).disableCache().buildWithTestScope()
    ...
    @Test
    fun testCachedStreamsWithRefreshAndSlowCollection() =
        testScope.runTest {
            // Given a cache and no source of truth, when 3 cached streams are requested with refresh
            // and the 1st stream has a slow collection, then the 1st stream gets 3 fetch updates
            // and the other streams get the cache result and the fetch result.
    ...
    fun testCachedStreamsWithRefreshAndFetchUpdates() =
        testScope.runTest {
            // Given a cache and no source of truth, when 2 cached streams are requested with refresh,
            // then the first stream gets 2 fetch updates, and the second stream gets the cache result
            // and the fetch result.
