...
class FetcherResponseTests {
  ...
  fun givenAFetcherThatThrowsAnExceptionInInvokeWhenStreamingThenTheExceptionsShouldNotBeCaught() =
    testScope.runTest {
      val store =
        StoreBuilder.from(Fetcher.ofResult { throw RuntimeException("don't catch me") })
          .buildWithTestScope()
  ...
  fun givenAFetcherThatEmitsErrorAndDataWhenSteamingThenItCanEmitValueAfterAnError() =
    testScope.runTest {
        ...
        StoreBuilder.from(
            fetcher =
              Fetcher.ofResultFlow { key: Int ->
                flowOf(FetcherResult.Error.Exception(exception), FetcherResult.Data("$key"))
              }
          )
          .buildWithTestScope()
      ...
      store.stream(StoreReadRequest.fresh(1)).test {
        ...
        assertEquals(StoreReadResponse.Data("1", StoreReadResponseOrigin.Fetcher()), awaitItem())
      }
    }
  ...
  fun givenTransformerWhenRawValueThenUnwrappedValueReturnedAndValueIsCached() =
    testScope.runTest {
      ...
      pipeline.stream(StoreReadRequest.cached(3, refresh = false)).test {
        ...
        assertEquals(
          StoreReadResponse.Data(value = 9, origin = StoreReadResponseOrigin.Fetcher()),
          awaitItem(),
        )
      }
      ...
      pipeline.stream(StoreReadRequest.cached(3, refresh = false)).test {
        assertEquals(
          StoreReadResponse.Data(value = 9, origin = StoreReadResponseOrigin.Cache),
          awaitItem(),
        )
      }
    }
  ...
  fun givenTransformerWhenErrorMessageThenErrorReturnedToUserAndErrorIsNotCached() =
    testScope.runTest {
      ...
      pipeline.stream(StoreReadRequest.cached(3, refresh = false)).test {
        ...
        assertEquals(
          StoreReadResponse.Data(value = 1, origin = StoreReadResponseOrigin.Fetcher()),
          awaitItem(),
        )
      }
    }
  ...
  fun givenTransformerWhenErrorExceptionThenErrorReturnedToUserAndErrorIsNotCached() =
    testScope.runTest {
      ...
      pipeline.stream(StoreReadRequest.fresh(3)).test {
        ...
        assertEquals(
          StoreReadResponse.Error.Exception(error = e, origin = StoreReadResponseOrigin.Fetcher()),
          awaitItem(),
        )
      }
  ...
  fun givenAFetcherThatEmitsCustomErrorWhenStreamingThenCustomErrorShouldBeEmitted() =
    testScope.runTest {
      ...
      val store =
        StoreBuilder.from(
            fetcher =
              Fetcher.ofResultFlow { _: Int -> flowOf(FetcherResult.Error.Custom(customError)) }
          )
          .buildWithTestScope()
